...
export function manipulateDate(dateTimeString, value, unit, format) {
  ...
  return moment(`${year}-${month}-${day}T${hours}:${minutes}:${seconds}`)
    .add(value, unit)
    .format(format)
}
...
export function formatFilterDates(date) {
  ...
  switch (date) {
    ...
    case `${filterTransactionDates.THIS_WEEK} (${moment()
      .startOf('week')
      .format('L')} to ${moment()
      .endOf('week')
      .format('L')})`:
    ...
    case `${filterTransactionDates.THIS_MONTH} (${moment()
      .startOf('month')
      .format('L')} to ${moment()
      .endOf('month')
      .format('L')})`:
...
export const generatePlaceTerms = (placeTerms, book) => {
  let targetPlaceTerm = placeTerms.find(
...
export const generateBookPayload = book => {
  return selectValidProps(
    [{ ...book, bookType: book.description, rule4DeductionEnabled: book.rule4 }],
    validBookProps
  )[0]
}
...
export const getOpponentDetails = (opponentId, opponentsArray) => {
  ...
  opponentsArray.forEach(opponent => {
    if (opponent.id == opponentId) OpponentDetails = opponent
  })
