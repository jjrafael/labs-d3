...
const time24 = value =>
  value && !/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/i.test(value) ? 'Invalid time format' : undefined
const alphanumericunderscore = value =>
  value && !/^[A-Za-z0-9\_]+$/i.test(value) ? 'Only letters, numbers and underscore are allowed' : undefined
...
const handicap = value =>
  value && !/^[+-]?[\d]+.?((0|00|25|5|50|75))?$/g.test(value) ? 'Invalid handicap format' : undefined
const incompleteHandicap = value =>
  value && !/^[+-]?[\d]+.?((2|7|))?$/g.test(value) ? 'Invalid handicap format' : undefined
...
const dateFormat = value =>
  value && !/^(0[1-9]|1[012])[- /.](0[1-9]|[12][0-9]|3[01])[- /.](19|20)\d\d$/.test(value)
    ? 'Invalid date format'
    : undefined
const maxNumber = max => value =>
  value && parseFloat(value) > parseFloat(max) ? `The maximum value allowed is ${formatNumber(max)}` : undefined

// allows user to enter incomplete handicap like 1.2 -> 1.25 or 1.7 -> 1.75
export {
  required,
  minLength,
  maxLength,
